...
void	MapGraphLoad()
{
	lpspr_map[0].png	=	LoadGraph(L"graphic/0.PNG");	//床（木材）
	lpspr_map[1].png	=	LoadGraph(L"graphic/1.PNG");	//床（絨毯）
	lpspr_map[2].png	=	LoadGraph(L"graphic/2.PNG");	//トゲ（左向き）
	lpspr_map[3].png	=	LoadGraph(L"graphic/3.PNG");	//トゲ（右向き）
	lpspr_map[4].png	=	LoadGraph(L"graphic/4.PNG");	//床（木材・暗い）
	lpspr_map[5].png	=	LoadGraph(L"graphic/5.PNG");	//床（絨毯）
	lpspr_map[6].png	=	LoadGraph(L"graphic/6.PNG");	//床（左向きトゲ用）
	lpspr_map[7].png	=	LoadGraph(L"graphic/7.PNG");	//床（右向きトゲ用）
	lpspr_map[11].png	=	LoadGraph(L"graphic/11.PNG");	//壁（右下）
	lpspr_map[12].png	=	LoadGraph(L"graphic/12.PNG");	//壁（右上）
	lpspr_map[13].png	=	LoadGraph(L"graphic/13.PNG");	//壁（左下）
	lpspr_map[14].png	=	LoadGraph(L"graphic/14.PNG");	//壁（左上）
	lpspr_map[15].png	=	LoadGraph(L"graphic/15.PNG");	//壁（右）
	lpspr_map[16].png	=	LoadGraph(L"graphic/16.PNG");	//壁（左）
	lpspr_map[17].png	=	LoadGraph(L"graphic/17.PNG");	//壁（下）
	lpspr_map[18].png	=	LoadGraph(L"graphic/18.PNG");	//壁（上）
	lpspr_map[19].png	=	LoadGraph(L"graphic/19.PNG");	//壁（  ￣| ）
	lpspr_map[20].png	=	LoadGraph(L"graphic/20.PNG");	//壁（ |￣ ）
	lpspr_map[21].png	=	LoadGraph(L"graphic/21.PNG");	//壁（  ＿| ）
	lpspr_map[22].png	=	LoadGraph(L"graphic/22.PNG");	//壁（ |＿ ）
	lpspr_map[23].png	=	LoadGraph(L"graphic/23.PNG");
	lpspr_map[24].png	=	LoadGraph(L"graphic/24.PNG");
	lpspr_map[25].png	=	LoadGraph(L"graphic/25.PNG");
	lpspr_map[26].png	=	LoadGraph(L"graphic/26.PNG");
	lpspr_map[27].png	=	LoadGraph(L"graphic/27.PNG");
	lpspr_map[28].png	=	LoadGraph(L"graphic/28.PNG");
	lpspr_map[29].png	=	LoadGraph(L"graphic/29.PNG");
	lpspr_map[30].png	=	LoadGraph(L"graphic/30.PNG");
	lpspr_map[31].png	=	LoadGraph(L"graphic/31.PNG");
	lpspr_map[32].png	=	LoadGraph(L"graphic/32.PNG");
	lpspr_map[33].png	=	LoadGraph(L"graphic/33.PNG");
	lpspr_map[34].png	=	LoadGraph(L"graphic/34.PNG");
	lpspr_map[35].png	=	LoadGraph(L"graphic/35.PNG");
	lpspr_map[36].png	=	LoadGraph(L"graphic/36.PNG");
	lpspr_map[37].png	=	LoadGraph(L"graphic/37.PNG");
}
...
void	MapDraw(int map[map_h][map_w],int map2[map_h][map_w],int map3[map_h][map_w],int map4[map_h][map_w])
{
	for(i = 0;i < map_h;i++)
	{
		...
		{
			//map[map_h][map_w]
			DrawGraph(map_x[i][e],map_y[i][e],lpspr_map[map[i][e]].png,TRUE);
			//map2[map_h][map_w]
			DrawGraph((map_x[i][e] + WINDOW_SIZE_W),map_y[i][e],lpspr_map[map2[i][e]].png,TRUE);
			//map3[map_h][map_w]
			DrawGraph((map_x[i][e] + (WINDOW_SIZE_W * 2)),map_y[i][e],lpspr_map[map3[i][e]].png,TRUE);
			//map4[map_h][map_w]
			DrawGraph((map_x[i][e] + (WINDOW_SIZE_W * 3)),map_y[i][e],lpspr_map[map4[i][e]].png,TRUE);
		}
	}
}
...
int	damage_trap(ADV_MAIN	*lpspr,CHARA_MAIN player,int map_c)
{
	...
	if(player.hp > 3)
	{
		if(lpspr->dmg == 1)
		{
			LoadDivGraph(L"graphic/player_dmg.PNG",lpspr->allnum,lpspr->x_num,lpspr->y_num,lpspr->w,lpspr->h,lpspr->sprNo);
		}
		if((dmg_c == player.hp) && (dmg == 0) && (lpspr->dmg != 0))
		{
			lpspr->dmg = 0;
			LoadDivGraph(L"graphic/player.PNG",lpspr->allnum,lpspr->x_num,lpspr->y_num,lpspr->w,lpspr->h,lpspr->sprNo);
		}
	}
	dmg_c = 0;
	return player.hp;
