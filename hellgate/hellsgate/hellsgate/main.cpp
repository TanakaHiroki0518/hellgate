	...
	while(!ProcessMessage() && !ClearDrawScreen() && !GetHitKeyStateAll( Key ) && !Key[KEY_INPUT_ESCAPE])
	{
		...
		else if(game_c == 2)
		{
			...
			if(play_c == 2)
			{
				...
				MapGraphLoad();
				MapInit();
				PlayMusic(L"sound/game_sound.MP3",DX_PLAYTYPE_LOOP);
				SetVolumeMusic(1);
				map_c	= 2;
		...
		else if(game_c == 3)
		{
			...
			if(scr_c == 0)
			{
				if(say == 0)
				{
					if((lpsprEn[0][0]->dmg == 0) && (lpsprEn[0][1]->dmg == 0) && (lpsprEn[1][0]->dmg == 0) && (lpsprEn[1][1]->dmg == 0))
					{
						...
						say = atari_damage(lpsprPl,player,map_c);
			...
			if((map_c - (map_c / 10 * 10) == 4))
			{
				if((dash_c == 0) || (dash_c == 3))
				{
					...
					string_c = key_main(player,lpsprPl,Key,map_c);
				}
...
int game_init()
{
	...
	for(i = 0;i < enemy_h;i++)
	{
		for(e = 0;e < enemy_w;e++)
		{
			graph_h = maptip_size;
			graph_w = maptip_size;
			lpsprEn[i][e] = spr_create(L"graphic/enemy.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
			if(lpsprEn[i][e]	==	NULL)
			{
				return	(-1);
			}
	...
	lpsprTi = spr_create(L"graphic/title.PNG" , TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprTi == NULL)
	{
		return (-1);
	}
	...
	lpsprBlk = spr_create(L"graphic/black.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprBlk == NULL)
	{
		return (-1);
	}
	...
	lpsprend = spr_create(L"graphic/prologue_end.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprend == NULL)
	{
		return (-1);
	}
	...
	lpsprGate = spr_create(L"graphic/hellgate.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprGate == NULL)
	{
		return (-1);
	}
	...
	graph_h = maptip_size;
	graph_w = maptip_size;
	lpsprStar = spr_create(L"graphic/star.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprStar == NULL)
	{
		return (-1);
	}
	...
	lpsprPl	=	spr_create(L"graphic/player.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprPl	==	NULL)
	{
		return	(-1);
	}
	...
	lpsprBap	=	spr_create(L"graphic/baphomet.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprBap	==	NULL)
	{
		return	(-1);
	}
	...
	graph_h = 150;
	graph_w = 250;
	lpsprMn	=	spr_create(L"graphic/menue_top.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprMn	==	NULL)
	{
		return	(-1);
	}
	...
	graph_h = 300;
	graph_w = 250;
	lpsprMn_up	=	spr_create(L"graphic/menue_big_window.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprMn_up	==	NULL)
	{
		return	(-1);
	}
	...
	graph_h	= 200;
	graph_w	= 800;
	lpsprMn_down	=	spr_create(L"graphic/menue_down.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	...
	lpsprYa	=	spr_create(L"graphic/menue_ya.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprYa	==	NULL)
	{
		return	(-1);
	}
	...
	graph_h	= graph_w	= 24;
	lpsprYa2	=	spr_create(L"graphic/menue_ya.PNG",	TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprYa2	==	NULL)
	{
		return	(-1);
	}
	...
	graph_h	= 100;
	graph_w	= 150;
	lpsprWi		=	spr_create(L"graphic/menue_window.PNG",TRUE,graph_h,graph_w,allnum,xnum,ynum);
	if(lpsprWi == NULL)
	{
		return	(-1);
	}
